...
public class KeyAuthFilter extends AbstractGatewayFilterFactory<KeyAuthFilter.Config>{
    ...
    private Mono<Void> handleException(ServerWebExchange exchange , String message , HttpStatus status){
        ...
        String errorReponse = String.format(
                "{\"timesamp\":\"%s\",\"status\":%d,\"error\":\"%s\",\"message\":\"%s\",\"path\":\"%s\"}",
                java.time.ZonedDateTime.now().toString(),
                status.value(),
                status.getReasonPhrase(),
                message,exchange.getRequest().getURI().getPath()
        );
        return response.writeWith(Mono.just(response.bufferFactory().wrap(errorReponse.getBytes())));
    }
